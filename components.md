...
如果你习惯 Element Plus 官方图标的使用方式，也可以安装 `@element-plus/icons-vue` 依赖，并按照官方的使用方式引入图标，点击[查看](http://element-plus.org/zh-CN/component/icon.html)详细介绍。
:::

:::warning 替换组件库
如果你不喜欢 Element Plus ，或者你想要将老项目迁移到 Kpu-admin 上，但老项目里使用的组件库并不是 Element Plus ，框架也提供了[其他 UI 组件库](/guide/other-ui)的支持。
:::
...
内建组件存放在 `packages/components/src/` 目录下，并通过 `@kpu-admin/components` 及对应 resolver 提供自动引入。
