...
### KaAnimatedCountToGroup

| 属性 | 说明 |
|------|------|
| 无 props | 仅用于包裹多个 KaAnimatedCountTo 组件 |
...
<template>
  <!-- 货币格式 -->
  <KaAnimatedCountTo :value="1234.56" :format="{ style: 'currency', currency: 'CNY' }" />

  <!-- 百分比格式 -->
  <KaAnimatedCountTo :value="0.75" :format="{ style: 'percent' }" />

  <!-- 带前缀后缀 -->
  <KaAnimatedCountTo :value="100" prefix="¥" suffix=" 元" />
</template>
```
...
<template>
  <KaAnimatedCountToGroup>
    <div>
      <KaAnimatedCountTo :value="100" />
    </div>
    <div>
      <KaAnimatedCountTo :value="200" />
    </div>
  </KaAnimatedCountToGroup>
</template>
```
...
<template>
  <div class="flex items-center gap-2">
    <KaAnimatedCountTo :value="1234" :trend="1" />
    <KaIcon name="i-material-symbols:trending-up" class="text-green-500" />
  </div>
</template>
```
