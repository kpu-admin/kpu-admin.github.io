...
<template>
  <KaTree :data="data" @selection-change="handleSelectionChange" />
  <div class="mt-4">
...
<template>
  <KaTree :data="data" checkable @check-change="handleCheckChange" />
  <div class="mt-4">
    选中节点：
...
```vue
<template>
  <KaTree :data="data" filterable filter-placeholder="搜索节点..." />
</template>
```
...
<template>
  <KaTree :data="data" filterable :filter-method="customFilter" />
</template>
```
...
```vue
<template>
  <KaTree :data="data" default-expand-all />
</template>
```
...
const treeRef = ref<InstanceType<typeof KaTree>>()
...
<template>
  <KaTree ref="treeRef" :data="data" node-key="id" />
  <KaButton @click="expandChild">展开子节点 1</KaButton>
</template>
```
  ...
  <KaTree :data="data">
    <template #default="{ node, expanded }">
      <div class="flex-between w-full">
        ...
        <KaBadge v-if="node.count" :value="node.count" />
      </div>
    </template>
  </KaTree>
</template>
...
<template>
  <div class="flex gap-2 mb-4">
    <KaButton @click="expandAll">展开全部</KaButton>
    <KaButton @click="collapseAll">收起全部</KaButton>
    <KaButton @click="expandSpecific">展开指定</KaButton>
  </div>
  <KaTree ref="treeRef" :data="data" node-key="id" />
</template>
```
...
<template>
  <KaButton @click="toggleCheckAll">
    {{ isAllChecked ? '取消全选' : '全选' }}
  </KaButton>
  <KaTree ref="treeRef" :data="data" checkable />
</template>
```
