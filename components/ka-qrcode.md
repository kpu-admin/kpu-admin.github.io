...
<template>
  <KaQrcode :value="qrValue" :size="200" />
</template>
```
...
```vue
<KaQrcode :value="'https://example.com'" :size="200" />
```
...
<template>
  <!-- 本地图片 Logo -->
  <KaQrcode :value="url" :size="200" :logo="logoSvg" />

  <!-- 网络图片 Logo -->
  <KaQrcode :value="url" :size="200" logo="https://example.com/logo.png" />
</template>
```
...
```vue
<!-- 加载中状态 -->
<KaQrcode :value="url" status="loading" />

<!-- 已过期状态 -->
<KaQrcode :value="url" status="expired" />

<!-- 自定义状态渲染 -->
<KaQrcode :value="url" status="scanned">
...
```vue
<script setup lang="ts">
const qrRef = ref<InstanceType<typeof KaQrcode>>()
...
<template>
  <KaQrcode ref="qrRef" :value="url" />
  <KaButton @click="downloadQr">下载二维码</KaButton>
</template>
```
