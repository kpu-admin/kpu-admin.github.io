...
<template>
  <KaTextarea v-model="value" placeholder="请输入内容..." />
</template>
```
...
<template>
  <KaTextarea v-model="value" placeholder="请输入描述..." rows="4" />
</template>
```
...
<template>
  <KaTextarea v-model="value" placeholder="请输入内容">
    <template #start>
      <KaIcon name="i-lucide:message-square" />
    </template>
  </KaTextarea>
</template>
```
...
<template>
  <KaTextarea v-model="value" placeholder="请输入内容（最多 200 字）" rows="4">
    <template #end>
...
```vue
<template>
  <KaTextarea v-model="value" rows="6" class="h-200" />
</template>
```
...
<template>
  <form @submit.prevent="handleSubmit" class="flex flex-col gap-4">
    <KaInput v-model="form.title" placeholder="标题" />
    <KaTextarea v-model="form.content" placeholder="内容描述..." rows="5" />
    <KaButton type="submit">提交</KaButton>
  </form>
</template>
```
...
```vue
<template>
  <KaTextarea v-model="value" disabled placeholder="禁用状态" />
</template>
```
