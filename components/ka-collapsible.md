...
<template>
  <KaCollapsible v-model="open">
    <template #trigger="{ open }">
      <KaButton variant="ghost">点击{{ open ? '收起' : '展开' }}</KaButton>
    </template>
    <div class="mt-2">
      这是可折叠的内容区域。
    </div>
  ...
  <KaCollapsible v-model="open">
    <template #trigger>
      <div class="flex items-center justify-between p-4 border rounded-lg cursor-pointer" @click="open = !open">
        <span>点击展开详情</span>
        <KaIcon :name="open ? 'i-lucide:chevron-up' : 'i-lucide:chevron-down'" />
      </div>
    </template>
...
<template>
  <div class="space-y-2">
    <KaCollapsible v-for="(faq, index) in faqs" :key="index" v-model="faqs[index].open">
      <template #trigger>
        <div 
        ...
        >
          ...
          <KaIcon :name="openIndex === index ? 'i-lucide:chevron-up' : 'i-lucide:chevron-down'" />
        </div>
      </template>
