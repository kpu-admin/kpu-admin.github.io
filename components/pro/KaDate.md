---
url: /components/pro/KaDate.md
---
# KaDate 日期

基于 `KaCalendarDate`、`KaCalendarMonth`、`KaCalendarYear` 组合的日期组件。默认渲染面板，设置 `picker` 后渲染为按钮触发的弹层选择器。月视图和年视图只用于切换当前展示锚点，不会提交日期值。
...
5. 范围选择请使用已有范围组件；`KaDate` 不提供 `range` prop。
