---
url: /components/pro/KaCalendarDate.md
---
# KaCalendarDate 日期日历
...
默认 `picker=false`，组件仍渲染日期面板。设置 `picker` 后会内置 `KaPopover` 与 `KaButton`，渲染为按钮触发的选择器。
...
1. 组件支持单日期和多日期选择；范围选择请使用 `KaCalendarDateRange`。
