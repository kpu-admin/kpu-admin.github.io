---
url: /components/pro/KaCalendarMonth.md
---
# KaCalendarMonth 月份日历
...
默认 `picker=false`，组件仍渲染月份面板。设置 `picker` 后会内置 `KaPopover` 与 `KaButton`，渲染为按钮触发的选择器。
...
1. 组件支持单月份和多月份选择；范围选择请使用 `KaCalendarMonthRange`。
