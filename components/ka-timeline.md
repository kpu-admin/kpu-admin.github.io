...
<template>
  <KaTimeline :data="data" merge>
    <template #default="{ item }">
      <KaCard>{{ item.content }}</KaCard>
    </template>
  </KaTimeline>
</template>
```
...
<template>
  <KaTimeline :data="data" class="h-400">
    <template #default="{ item }">
      <KaCard>{{ item.content }}</KaCard>
    </template>
  </KaTimeline>
...
<template>
  <KaTimeline :data="projectMilestones">
    <template #default="{ item }">
      <KaCard>
        <template #header>
...
<template>
  <div>
    ...
    <KaButton @click="addActivity('新活动', '当前用户')">添加活动</KaButton>
  </div>
</template>
```
