---
url: /components/ka-context-menu.md
---
# KaContextMenu 右键菜单组件
  ...
  <table class="w-full">
    ...
    <tbody>
      <tr v-for="row in data" :key="row.id" class="hover:bg-muted/50">
        <KaContextMenu :items="getMenuItems(row)">
...
<template>
  <div class="space-y-2">
    ...
    >
      <KaContextMenu :items="getFileMenu(file)">
        <div class="flex items-center gap-3 flex-1">
          <KaIcon name="i-lucide:file-text" />
          <span>{{ file.name }}</span>
        </div>
      </KaContextMenu>
    </div>
