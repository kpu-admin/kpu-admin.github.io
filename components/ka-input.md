...
<template>
  <KaInput v-model="value" placeholder="请输入..." />
</template>
```
...
<template>
  <KaInput v-model="value" placeholder="请输入内容" />
</template>
```
...
<template>
  <KaInput v-model="password" type="password" placeholder="请输入密码" />
</template>
```
...
<template>
  <KaInput v-model="value" clearable @clear="handleClear" />
</template>
```
...
```vue
<template>
  <KaInput v-model="value" placeholder="搜索...">
    <template #start>
...
```vue
<template>
  <KaInput v-model="value" placeholder="请输入验证码">
    <template #end>
      <KaButton size="sm">获取</KaButton>
    </template>
  </KaInput>
</template>
```
...
```vue
<template>
  <KaInput v-model="value">
    <template #start>
      <KaIcon name="i-lucide:user" />
    </template>
    <template #end>
      <KaIcon name="i-lucide:check-circle" class="text-green-500" />
    </template>
  </KaInput>
</template>
```
...
```vue
<template>
  <KaInput v-model="value" disabled placeholder="禁用状态" />
</template>
```
...
<template>
  <KaInput v-model="keyword" clearable placeholder="搜索..." @keyup.enter="handleSearch">
    <template #start>
      <KaIcon name="i-lucide:search" />
    </template>
  </KaInput>
...
<template>
  <div>
    <KaInput v-model="password" type="password" clearable placeholder="请输入密码" />
