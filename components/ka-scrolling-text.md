---
url: /components/ka-scrolling-text.md
---
# KaScrollingText 滚动文字组件
...
<template>
  <KaScrollingText :items="announcements" />
</template>
```
...
```vue
<KaScrollingText :items="hotSearches">
  <template #item="{ item, index }">
    <div class="flex items-center gap-2">
      <span :class="getRankClass(index)">{{ index + 1 }}</span>
      <span>{{ item }}</span>
      <KaBadge v-if="index < 3" value="hot" />
    </div>
  </template>
</KaScrollingText>
```
...
<template>
  <KaScrollingText :items="links" @item-click="handleItemClick" />
</template>
```
...
```vue
<!-- 鼠标移入时自动暂停滚动，移出后继续 -->
<KaScrollingText :items="messages" />
```
