...
<template>
  <KaLayoutContainer v-model:left-side-visiable="leftVisible">
    <template #leftSide>可折叠的左侧栏</template>
    <template #default>主内容</template>
  </KaLayoutContainer>
  
  <KaButton @click="leftVisible = !leftVisible">
    切换左侧栏
  </KaButton>
</template>
```
...
```vue
<template>
  <KaLayoutContainer :hide-left-side-toggle="true">
