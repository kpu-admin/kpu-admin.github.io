...
```vue
<template>
  <KaMarquee :pause-on-hover="true">
    <KaCard v-for="review in reviews" :key="review.id" class="w-80 p-4 mx-2">
...
```vue
<template>
  <KaMarquee :pause-on-hover="true" :repeat="6">
...
```vue
<template>
  <KaMarquee vertical :pause-on-hover="true" class="h-64">
    <div v-for="notice in notices" :key="notice.id" class="py-3 border-b">
      <KaIcon name="i-heroicons:bell" class="text-primary" />
...
```vue
<template>
  <KaMarquee :reverse="true" class="[--duration:60s]">
