...
```vue
<template>
  <KaSparkline :value="[10, 20, 15, 25, 30, 20, 35]" />
</template>
```
...
```vue
<template>
  <KaSparkline :value="[10, 20, 15, 25, 30]" :width="150" :height="50" />
</template>
```
...
<template>
  <KaSparkline :value="data" :tooltip="true" />
</template>
```
  ...
  <table class="w-full">
    ...
    <tbody>
      <tr v-for="product in products" :key="product.name">
        ...
        <td>{{ product.sales }}</td>
        <td>
          <KaSparkline :value="product.trend" :width="80" :height="30" />
        </td>
      </tr>
    </tbody>
  </table>
