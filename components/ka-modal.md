...
<template>
  <div class="flex gap-2">
    <KaButton @click="showAlert">信息提示</KaButton>
    <KaButton @click="showSuccess">成功提示</KaButton>
    <KaButton @click="showConfirm">确认对话框</KaButton>
    <KaButton @click="showError">错误提示</KaButton>
    <KaButton @click="showWarning">警告提示</KaButton>
  </div>
</template>
```
...
<template>
  <KaButton @click="open = true">打开对话框</KaButton>
  <KaModal v-model="open" title="对话框标题">
    <div>对话框内容</div>
  </KaModal>
</template>
```
...
<template>
  <KaButton @click="open = true">打开对话框</KaButton>
  <KaModal v-model="open" title="对话框标题" description="这里是一段描述">
    <div>这是对话框内容</div>
  </KaModal>
</template>
```
...
<template>
  <div class="flex gap-2">
    <KaButton @click="successOpen = true">成功提示</KaButton>
    <KaButton @click="errorOpen = true">错误提示</KaButton>
  </div>
  
  <KaModal v-model="successOpen" title="操作成功" icon="success" align-center border>
    <div>您的操作已成功完成</div>
  </KaModal>
  
  <KaModal v-model="errorOpen" title="操作失败" icon="error" align-center border>
    <div>操作失败，请稍后重试</div>
  </KaModal>
</template>
```
...
```vue
<template>
  <KaModal v-model="open" title="可拖拽" draggable header>
...
```vue
<template>
  <KaModal v-model="open" title="可最大化" maximizable>
...
```vue
<template>
  <KaModal v-model="open" title="居中对话框" align-center>
...
```vue
<template>
  <KaModal v-model="open" title="内容居中" center>
...
```vue
<script setup lang="ts">
import { useKaModal } from '@/ui/components/modal'
...
<template>
  <KaModal v-model="open" title="编辑" :before-close="handleBeforeClose">
    <form class="flex flex-col gap-4">
      <KaInput placeholder="请输入..." />
    </form>
  </KaModal>
</template>
```
...
<template>
  <KaModal v-model="open" title="提交" :loading="loading" :confirm-button-loading="loading" @confirm="handleSubmit">
    <form class="flex flex-col gap-4">
      <KaInput placeholder="输入内容" />
    </form>
  </KaModal>
</template>
```
...
```vue
<template>
  <KaModal v-model="open" title="大对话框" class="sm:max-w-2xl">
    <div>这是一个较宽的对话框</div>
  </KaModal>
</template>
```
...
6. **函数式调用**：`useKaModal()` 提供 `info`、`success`、`warning`、`error`、`confirm` 方法
