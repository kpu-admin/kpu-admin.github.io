...
推荐去[阿里巴巴矢量图标库](https://www.iconfont.cn/)下载图标，将准备好的 SVG 图标文件放到 `apps/<app>/src/assets/icons/` 目录下，然后在页面中就可以通过 `KaIcon` 组件使用了，name 就是 svg 的文件名。
...
当然你同样也可以通过 `KaIcon` 组件使用它。

```vue
<KaIcon name="i-ep:arrow-right" />
```
...
当然这么使用并没有很方便，依旧还是需要手动导入一个 Icon 组件。如果你也觉得麻烦的话，那么你可以使用 `KaIcon` 组件来展示，框架已经帮你做好的所有处理。

```vue
<KaIcon name="ep:arrow-right" />
```
...
框架提供了一份解决方案，在命令行执行 `pnpm run -F <app> generate:icons` ，按照指引选择你需要用到的图标集（此处选择的图标集也是图标选择器里展示的图标集），并选择使用方式为离线。这样再在框架中使用这些图标，就不会触发外部网络请求了，但如果使用选择之外的图标，依旧还是会触发外部网络请求。
