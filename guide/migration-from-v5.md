...
```ts {twoslash}
import { setSettingsLegacy } from '@kpu-admin/settings'
...
* `KaAuth` → `AppAuth`
* `KaBackToTop` → `AppBackToTop`
* `KaCheckUpdates` → `AppCheckUpdates`
* `KaCopyright` → `AppCopyright`
* `KaNotAllowed` → `AppNotAllowed`
* `KaSystemInfo` → `AppSystemInfo`

其中通常只需要重点关注 `KaAuth`，因为它更可能被业务页面直接使用。

### 组件名称调整

* `KaPinInput` → `KaInputOTP`

### 移除组件

* `KaNotification` 已移除

如果你的旧项目里直接使用过 `KaNotification`，需要在迁移时手动删除或替换。
...
* RTL 模式改为跟随语言设置自动切换，可在 `apps/<app>/src/locales/index.ts` 导出的 `localesInfo` 中配置各语言的 `direction`
