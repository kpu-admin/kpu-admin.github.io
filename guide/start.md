...
kpu-admin/
├── apps/              # 应用目录
│   ├── core           # 应用源码（集成 Element Plus 组件库）
│   ├── core-*         # 应用源码（集成其他的 UI 组件库）
│   └── example        # 示例应用
├── packages/          # 公共包目录
├── docs/              # 文档站点
├── scripts/           # 脚本工具
...
其他 UI 组件库的介绍可以点[这里](/guide/other-ui)。
...
```bash
# 运行指定应用
pnpm -F @kpu-admin/core dev

# 构建指定应用
pnpm -F @kpu-admin/core build

# 在指定应用下执行任意命令
pnpm -F @kpu-admin/core lint
```
...
```bash
# 为指定应用添加依赖
pnpm add axios -F @kpu-admin/core
