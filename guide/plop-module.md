...
```vue-html {1}
<KaPageHeader :title="route.name === 'routerName' ? '新增演示' : '编辑演示'">
  <KaButton variant="outline" size="sm" class="rounded-full" @click="onCancel">
    <KaIcon name="i-ep:arrow-left" />
    返回
  </KaButton>
</KaPageHeader>
```
...
紧接着的是下面这段代码，它的作用是当列表页开启缓存时，并以路由跳转的方式进入详情页进行新增或编辑操作后，先对缓存的列表页进行一次数据更新，然后再跳转回列表页，这样即可以保证列表页相关搜索、翻页等状态可以被缓存，又能保证数据是最新。具体实现则是通过绑定一个事件总线，并在详情页里调用这个事件。需要注意的是，如果开启标签栏，事件总线的名称得确保唯一。
