  ...
  if (shouldShow) {
    ...
    countdownInterval.value = setInterval(() => {
      ...
      if (currentTime < start) {
        ...
        text.value = `Kpu-admin 五周年庆即将开始<i style="margin-left: 20px; font-size: 0.75em;">距离活动开始还有 ${formatTime(distance)}</i>`
      }
      else {
        // 活动进行中倒计时
        ...
        text.value = `Kpu-admin 五周年庆，全年最低价，点击查看<i style="margin-left: 20px; font-size: 0.75em;">距离活动结束还有 ${formatTime(distance)}</i>`
      }
    }, 1000)
